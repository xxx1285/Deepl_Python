...
async def main(folder_path, output_folder):
    json_files = [f for f in os.listdir(folder_path) if f.endswith('.json')]

    for json_file in json_files:
        full_path = os.path.join(folder_path, json_file)
        output_json = os.path.join(output_folder, f"{os.path.splitext(json_file)[0]}_OUTPUT_09-02.json")
...
folder_path = r'Parsing\Poisk-Dropov\Brazil\Result_JSON\del3'  # Путь к исходному JSON-файлу
output_folder = r'Parsing\Poisk-Dropov\Brazil\Result_JSON\del3'  # Путь для нового JSON-файла
