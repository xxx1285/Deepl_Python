...
class ForbesUrlSpider(scrapy.Spider):
    name = 'correiodopovo'
    allowed_urls = ['correiodopovo.com.br']
    start_urls = ['https://www.correiodopovo.com.br/']
    custom_settings = {
        'DOWNLOAD_DELAY': 1,  # Затримка між запитами
        'CONCURRENT_REQUESTS_PER_URL': 15,  # Кількість одночасних запитів на один домен
        'USER_AGENT': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36',
        'HTTPCACHE_ENABLED': False,
        'COOKIES_ENABLED': False,
        'DOWNLOAD_TIMEOUT': 20,  # Таймаут на завантаження
    }
    exclude_words = ['wiki', 'google', 'forbes', '.gov', '.edu', 'livejournal', 'blogspot', 'wordpress', 'broadwayworld', 
                     ...
                     '.am', '.lv', '.de', '.az', '.us', '.ca', '.fi', '.no', '.hu', '.exe', '.pdf'
                     ]  # Слова для виключення з посилань
    excluded_paths = ['/home_', 'login', 'account.', '#', '@', 'twitter-com.', 'translate.goog', 'amazon.com', '.exe', '.pdf']  # Шляхи, які потрібно виключити
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.file_path = 'D:\\Gembling\\Deepl_Python\\Deepl_Python\\Parsing\\Poisk-Dropov\\Brazil\\Result_JSON\\3-urls-correiodopovo-v2.json'
