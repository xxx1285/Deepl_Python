...
SAVE_ALL_URLS = r'Parsing\Search_All_UrlsSite\output\allurls2.json'
    ...
    async with aiohttp.ClientSession() as session:
        while queue and links_visited < LIMIT_URLS:
            ...
            if url not in visited_urls:
                ...
                html = await fetch_html(session, url)
                if html:
                    title = await get_page_title(html)
                    cleaned_title = re.sub(r'[^\w\d, -]', '', title)
                    results[url] = cleaned_title
                    links_visited += 1

    with open(SAVE_ALL_URLS, 'w', encoding='utf-8') as json_file:
