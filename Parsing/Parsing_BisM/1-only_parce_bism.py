...
# Функция для получения альтернативной версии и product_id
# async def get_alternative_version_and_product_id(session, soup, current_url):
#     alternate_links = soup.find_all('link', rel='alternate')
#     alternate_url = next((link['href'] for link in alternate_links if link['href'] != current_url), None)
#     if alternate_url:
#         html = await fetch(session, alternate_url, asyncio.Semaphore(10))
#         alternate_soup = BeautifulSoup(html, 'lxml')
#         product_id_input = alternate_soup.find('input', {'id': 'product_id'})
#         alternative_product_id = product_id_input['value'] if product_id_input else 'Не найдено product_id'
#     else:
#         alternative_product_id = 'Не найдено alternative_product_id'
#     return alternative_product_id
...
async def parse_item_data(session, url, semaphore, section_name):
    ...
    # alternative_product_id = await get_alternative_version_and_product_id(session, soup, url)
    # print(alternative_product_id)
