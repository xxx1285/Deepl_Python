...
# Функція перевірки ключевих слів в
...
async def get_protocol(domain_name):
    async with semaphore, httpx.AsyncClient(verify=False, follow_redirects=False) as client: # створюємо асинхронного клієнта
        try:
            ...
            print(f"***************************************************Invalid URL for domain: {domain_name}")
            pass
        try:
            # await використовується для "очікування" результату асинхронної операції
            ...
            with open(os.path.join(folder_path, file), "r") as f:
                ...
                with open(output_file_name, "w") as output_file:
                    ...
                    async def process_domain(domain_name):
                        ...
                        protocol_http = await get_protocol(domain_name) # Використовуємо 'await', щоб дочекатися результату
                        print(str(protocol_http) + " - " + domain_name)

                        if protocol_http is not None:
                            domain_name = protocol_http + domain_name
                            async with semaphore, httpx.AsyncClient(timeout=7.0, verify=False, follow_redirects=False) as client: # використовуємо асинхронного клієнта
                                try:
                                    ...
                                    if response.status_code == 200:
                                        ...
                                        for subdir in possible_subdirectories:
                                            if success_counter >= 2: 
                                                break
                                            #  формуємо URL 
                                            url = domain_name.rstrip('/') + '/' + subdir    # rstrip('/') що видалить будь-який слеш з правого краю
                                            try:
                                                response = await client.get(url, headers=headers, timeout=10.0) # використовуємо асинхронний запит
                                                if response.status_code == 200:
                                                    success_counter += 1
                                                    if 'cate7456' in url:   # ???? скинути якщо дає код 200 на сміття різне 
                                                        break
