...
CID_MY_COMPANY = ['8207621176809025478', '13280552074301093291'] 
...
save_urls_CIT_result = r"GOOGLE\Google_maps_Adress_API\output\CID_URLs_Borisp_InstdivFreyameb_16-01-2024.txt"
        ...
        if data:
            ...
            with open(save_urls_CIT_result, 'w', encoding='utf-8') as file:
                for row in data[1:]:  # Обработка данных, начиная со второй строки
                    ...
                    for cid_mycompany in CID_MY_COMPANY:
                        if row_data['CID_target_num'] in CID_PLACE_TARGET and row_data['CID_my_company'] == cid_mycompany:
                            keywords = row_data['keyword'].split(', ')
                            cid_target_place = row_data['CID_target_num']
                            for keyword in keywords:
                                line = fun_cid_combi_url(keyword, cid_target_place, cid_mycompany)
                                file.write(line + '\n')
        else:
            print("Таблица пуста.")
