...
def categorize_url(url):
    ...
    if any(keyword in url_lower for keyword in profile_keywords):
        ...
        return "comment"
    elif any(keyword in url_lower for keyword in forum_blog_keywords):
        return "forum"
    return ""
...
def extract_unique_urls(text, excluded_words):
    ...
    for match in matches:
        ...
        if domain and domain not in excluded_words:
            # зберігаємо найперший зустрічний URL для кожного домена
            unique_pairs.setdefault(domain, url.lower())
...
if __name__ == "__main__":
    input_filename = r'Linkbilding\2_Find_unic_URl_in_Text_Chistim\input\baza_all_links.txt'
    output_filename = r'Linkbilding\2_Find_unic_URl_in_Text_Chistim\output\extracted_urls_4.csv'
