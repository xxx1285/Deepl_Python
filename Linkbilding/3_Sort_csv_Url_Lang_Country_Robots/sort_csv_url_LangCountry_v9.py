...
async def process_urls_chunk(urls_chunk, rows_chunk, output_filename):
    ...
    with open(output_filename, 'a', newline='', encoding='utf-8') as f:
        writer = csv.DictWriter(f, fieldnames=['Domain', 'URL', 'Category', 'Lang_Prefix', 'Language', 'Country', 'Status_code'])


        for (html_content, status), row in zip(results, rows_chunk):
...
async def main():
    ...
    output_filename = r"Linkbilding\3_Sort_csv_Url_Lang_Country_Robots\output\result_urls_4_2test.csv"
